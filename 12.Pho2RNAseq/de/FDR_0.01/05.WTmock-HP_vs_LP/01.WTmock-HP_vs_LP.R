...
setwd("/rds/user/cx264/hpc-work/project/2.Jeongmin/01.Saskia//4.DEanalysis/05.WTmock-HP_vs_LP/")
...
library(stringr)
library(statmod)
my_count_matrix <- read.csv("01.raw.count.WTmock-HP_vs_LP.tab", sep = "\t", header=T)
...
write.csv(DEG_glmQLF, "1.4.QLF.DE.results.WTmock-HP_vs_LP.FINAL.csv", quote = F)

#heatmap plot
deseq_results <- read.csv("1.4.QLF.DE.results.WTmock-HP_vs_LP.FINAL.csv",sep=",", header = T)
...
pdf("1.5.QLF.DiffGene.WTmock-HP_vs_LP.heatmap.pdf")
...
dev.off()
png("1.5.QLF.DiffGene.WTmock-HP_vs_LP.heatmap.png",width = 636, height = 980)
heatmap.2(data.matrix(diffGenes[,c(2,3,4,5,6,7,8,9)]),
    Rowv=as.dendrogram(clustRows),
    Colv=as.dendrogram(clustColumns),
    RowSideColors=module.color,
...
#volcano plot
pdf("1.6.QLF.DiffGene.WTmock-HP_vs_LP.volcano.pdf", 7, 7)
...
dev.off()
png("1.6.QLF.DiffGene.WTmock-HP_vs_LP.volcano.png", 800, 800)
EnhancedVolcano(deseq_results, lab = rownames(deseq_results), x = 'logFC', y = 'FDR', pCutoff = 10e-3, 
    FCcutoff=1.0, ylim = c(0, 5), xlim = c(-15, 15), pointSize = 1.0, labSize = 0, 
    colAlpha = 1, legendIconSize=2.0, legendLabSize = 10) + theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank()) 
dev.off()
