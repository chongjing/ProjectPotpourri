...
setwd("/rds/user/cx264/hpc-work/project/2.Jeongmin/01.Saskia//4.DEanalysis/07.WTmyc-HP_vs_LP/")
...
library(stringr)
library(statmod)
my_count_matrix <- read.csv("01.raw.count.WTmyc-HP_vs_LP.tab", sep = "\t", header=T)
...
write.csv(DEG_glmQLF, "1.4.QLF.DE.results.WTmyc-HP_vs_LP.FINAL.csv", quote = F)

#heatmap plot
deseq_results <- read.csv("1.4.QLF.DE.results.WTmyc-HP_vs_LP.FINAL.csv",sep=",", header = T)
...
pdf("1.5.QLF.DiffGene.WTmyc-HP_vs_LP.heatmap.pdf")
...
dev.off()
png("1.5.QLF.DiffGene.WTmyc-HP_vs_LP.heatmap.png",width = 636, height = 980)
heatmap.2(data.matrix(diffGenes[,c(2,3,4,5,6,7,8,9)]),
    Rowv=as.dendrogram(clustRows),
    Colv=as.dendrogram(clustColumns),
    RowSideColors=module.color,
