...
setwd("/rds/user/cx264/hpc-work/project/2.Jeongmin/01.Saskia//4.DEanalysis.FDR_0.05/01.WTLP-myc_vs_mock/")
...
library(stringr)
library(statmod)
my_count_matrix <- read.csv("../../4.DEanalysis.FDR_0.01/01.WTLP-myc_vs_mock/01.raw.count.WTLP-myc_vs_mock.tab", sep = "\t", header=T)
...
EnhancedVolcano(deseq_results, lab = rownames(deseq_results), x = 'logFC', y = 'FDR', pCutoff = 0.05, 
    FCcutoff=1.0, ylim = c(0, 8), xlim = c(-15, 15), pointSize = 1.0, labSize = 0, 
