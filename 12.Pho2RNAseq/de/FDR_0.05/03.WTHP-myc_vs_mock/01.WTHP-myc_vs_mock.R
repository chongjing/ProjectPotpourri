...
setwd("/rds/user/cx264/hpc-work/project/2.Jeongmin/01.Saskia//4.DEanalysis.FDR_0.05/03.WTHP-myc_vs_mock/")
...
library(stringr)
library(statmod)
my_count_matrix <- read.csv("../../4.DEanalysis.FDR_0.01/03.WTHP-myc_vs_mock/01.raw.count.WTHP-myc_vs_mock.tab", sep = "\t", header=T)
...
summary(decideTests(qlf,PValue=0.05))
#       group
#Down    12
#NotSig 24422
#Up      17
...
#volcano plot
pdf("1.7.QLF.DiffGene.WTHP-myc_vs_mock.volcano.pdf", 7, 7)
EnhancedVolcano(deseq_results, lab = rownames(deseq_results), x = 'logFC', y = 'PValue', pCutoff = 0.05, 
    FCcutoff=1.0, ylim = c(0, 3), xlim = c(-10, 10), pointSize = 1.0, labSize = 0, 
    colAlpha = 1, legendIconSize=2.0, legendLabSize = 10) + theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank()) 
dev.off()
png("1.7.QLF.DiffGene.WTHP-myc_vs_mock.volcano.png", 800, 800)
EnhancedVolcano(deseq_results, lab = rownames(deseq_results), x = 'logFC', y = 'PValue', pCutoff = 0.05, 
    FCcutoff=1.0, ylim = c(0, 3), xlim = c(-10, 10), pointSize = 1.0, labSize = 0, 
    colAlpha = 1, legendIconSize=2.0, legendLabSize = 10) + theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank()) 
dev.off()
